...
try {
    if ($action === 'entrar') {
        ...
        $stmt = $pdo->prepare("SELECT id, status FROM sessoes_quiz WHERE codigo = ?");
        ...
        if ($sessao['status'] !== 'aguardando') {
            echo json_encode(['sucesso' => false, 'erro' => 'Sala não aceita novos participantes no momento.']);
            exit;
        }
        ...
        // jogador solicita sair da sala
        $data = json_decode(file_get_contents('php://input'), true);
        $jogador_id = $data['jogador_id'] ?? ($_GET['jogador_id'] ?? null);
        if (!$jogador_id) {
            echo json_encode(['sucesso' => false, 'erro' => 'jogador_id ausente']);
            exit;
        }
        // marcar jogador como ausente (preserva pontuação)
        // garantir coluna 'ausente' existe
        $col = $pdo->prepare("SHOW COLUMNS FROM jogadores_sessao LIKE 'ausente'");
        $col->execute();
        if (!$col->fetch()) {
            $pdo->exec("ALTER TABLE jogadores_sessao ADD COLUMN ausente TINYINT(1) DEFAULT 0 AFTER ultimo_ping");
        }

        $stmt = $pdo->prepare("UPDATE jogadores_sessao SET ausente = 1 WHERE id = ?");
        $stmt->execute([$jogador_id]);
        echo json_encode(['sucesso' => true]);
    }
} catch (Exception $e) {
